...
function renderItem(item_text, id){
    ...
    trash_span.addEventListener("click", () => { //ref: https://developer.mozilla.org/en-US/docs/Web/API/EventTarget/addEventListener
        list_item.remove(); //add an even listener to span_trash. When clicked will remove list_item.

        //Lab7 Part 3 C
        items = items.filter(x => x.id !== id); // create copy of items list, excluding items with ['element id'='id variable']. ref: https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array/filter
